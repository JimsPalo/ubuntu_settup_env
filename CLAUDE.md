...
- **`install-packages.sh`** (run once with `sudo`) — apt packages, PPAs, GPG keys, system-level `systemctl enable/start`, and system-wide config files under `/etc` (e.g. forcing `WaylandEnable=false` in `/etc/gdm3/custom.conf` so touchegg gestures work by default). Nothing here touches a user's home directory or gsettings.
...
- Touchpad gestures require an **X11** session — Wayland is not supported by touchegg. Check `loginctl show-session $(loginctl list-sessions --no-legend | awk '{print $1}') -p Type` before debugging gesture issues further. `install-packages.sh` forces X11 at the greeter (see above), but a session already running under Wayland needs a logout/login (or `sudo systemctl restart gdm3`, which logs the user out immediately) to pick it up.
- `sudo` inside a Claude Code session has no TTY to prompt for a password, so any `sudo` command not already covered by cached credentials or passwordless config just fails with "a terminal is required to read the password" — it does **not** hang waiting for input. When a fix needs `sudo` (editing `/etc/gdm3/custom.conf`, restarting a system service, etc.), hand the user the exact command(s) to run themselves in a real terminal rather than attempting them here.
